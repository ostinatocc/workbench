...
from aionis_workbench.e2e.real_e2e.scenario_runner import (
    ...
    run_live_app_plan_scenario,
)


def test_live_app_plan_real_scenario(tmp_path) -> None:
    ...
    result = run_live_app_plan_scenario(
        repo_entry,
        cache_root=tmp_path / ".real-e2e-cache",
        launcher_home=tmp_path / ".aionis-live-home",
    )
    ...
    assert result.details["active_sprint_id"] == "sprint-1"
    assert result.details["active_sprint_proposed_by"] == "live_planner"
    assert result.details["product_title"]
    assert result.details["planning_rationale_count"] > 0
    assert result.details["negotiation_notes_count"] > 0
    ...
    assert result.details["app_plan_duration_seconds"] >= 0
    assert result.details["total_duration_seconds"] >= result.details["app_plan_duration_seconds"]
