...
from aionis_workbench.e2e.real_e2e.scenario_runner import (
    ...
    run_live_resume_complete_scenario,
)


def test_live_resume_complete_real_scenario(tmp_path) -> None:
    repo_entry = next(item for item in load_real_repo_manifest() if "live-run-resume" in item.scenario_tags)
    ...
    result = run_live_resume_complete_scenario(
        repo_entry,
        cache_root=tmp_path / ".real-e2e-cache",
        launcher_home=tmp_path / ".aionis-live-home",
    )
    ...
    assert result.details["doctor_mode"] == "live"
    assert result.details["run_runner"] == "run"
    assert result.details["run_status"] in {"paused", "needs_attention"}
    assert result.details["resume_runner"] == "resume"
    assert result.details["resume_status"] == "completed"
    assert result.details["resume_task_state_status"] == "completed"
    assert result.details["completed_session_status"] == "completed"
    ...
    assert result.details["run_duration_seconds"] >= 0
    assert result.details["resume_duration_seconds"] >= 0
    assert result.details["total_duration_seconds"] >= result.details["resume_duration_seconds"]
