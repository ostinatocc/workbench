...
from aionis_workbench.e2e.real_e2e.scenario_runner import (
    ...
    run_live_run_pause_scenario,
)


def test_live_run_pause_real_scenario(tmp_path) -> None:
    ...
    result = run_live_run_pause_scenario(
        repo_entry,
        cache_root=tmp_path / ".real-e2e-cache",
        launcher_home=tmp_path / ".aionis-live-home",
    )
    ...
    assert result.details["ready_output_first_line"].startswith("ready:")
    ...
    assert result.details["total_duration_seconds"] >= result.details["run_duration_seconds"]
