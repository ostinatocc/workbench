...
from aionis_workbench.e2e.real_e2e.scenario_runner import (
    ...
    run_live_app_retry_compare_scenario,
)


def test_live_app_retry_compare_real_scenario(tmp_path) -> None:
    ...
    result = run_live_app_retry_compare_scenario(
        repo_entry,
        cache_root=tmp_path / ".real-e2e-cache",
        launcher_home=tmp_path / ".aionis-live-home",
    )
    ...
    assert result.details["baseline_status"] == "failed"
    assert result.details["outcome_status"] in {"passed", "failed"}
    assert result.details["outcome_summary"]
    assert result.details["improvement_status"] in {"improved", "unchanged", "changed", "regressed"}
    assert result.details["loop_status"] in {"ready_for_next_sprint", "retry_available", "retry_budget_exhausted"}
    ...
    assert result.details["app_retry_duration_seconds"] >= 0
    assert result.details["app_qa_after_retry_duration_seconds"] >= 0
    assert result.details["total_duration_seconds"] >= result.details["app_qa_after_retry_duration_seconds"]
