...
from aionis_workbench.e2e.real_e2e.scenario_runner import (
    ...
    run_live_app_escalate_scenario,
)


def test_live_app_escalate_real_scenario(tmp_path) -> None:
    ...
    result = run_live_app_escalate_scenario(
        repo_entry,
        cache_root=tmp_path / ".real-e2e-cache",
        launcher_home=tmp_path / ".aionis-live-home",
    )
    ...
    assert result.details["recommended_next_action"] == "replan_or_escalate"
    assert result.details["provider_id"]
    assert result.details["model"]
    assert result.details["app_escalate_duration_seconds"] >= 0
    assert result.details["total_duration_seconds"] >= result.details["app_escalate_duration_seconds"]
