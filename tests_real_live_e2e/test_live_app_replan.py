...
from aionis_workbench.e2e.real_e2e.scenario_runner import (
    ...
    run_live_app_replan_scenario,
)


def test_live_app_replan_real_scenario(tmp_path) -> None:
    ...
    result = run_live_app_replan_scenario(
        repo_entry,
        cache_root=tmp_path / ".real-e2e-cache",
        launcher_home=tmp_path / ".aionis-live-home",
    )
    ...
    assert result.details["active_sprint_id"].startswith("sprint-1-replan-")
