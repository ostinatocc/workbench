...
from aionis_workbench.e2e.real_e2e.scenario_runner import (
    ...
    run_live_ab_test_compare_scenario,
)


def test_live_ab_test_report_real_scenario(tmp_path) -> None:
    ...
    result = run_live_ab_test_compare_scenario(
        repo_entry,
        cache_root=tmp_path / ".real-e2e-cache",
        launcher_home=tmp_path / ".aionis-live-home",
    )
    ...
    assert result.details["aionis_arm_scenario_id"] == "live-app-replan-generate-qa-advance"
    assert result.details["benchmark_scenario_id"] == "persistence-and-hydration"
    assert result.details["comparison"]["winner"] == "aionis"
    assert result.details["baseline"]["ended_in"] == "escalate"
    assert result.details["aionis"]["ended_in"] == "advance"
    assert result.details["aionis"]["provider_id"]
    assert result.details["aionis"]["model"]
    assert result.details["aionis"]["latest_convergence_signal"]
    assert result.details["benchmark_summary"]
    assert result.details["live_profile"]["latest_convergence_signal"]
    assert result.details["provider_id"]
    assert result.details["model"]
    assert result.details["total_duration_seconds"] > 0
