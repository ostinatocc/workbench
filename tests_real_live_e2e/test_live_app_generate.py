...
from aionis_workbench.e2e.real_e2e.scenario_runner import (
    ...
    run_live_app_generate_scenario,
)


def test_live_app_generate_real_scenario(tmp_path) -> None:
    ...
    result = run_live_app_generate_scenario(
        repo_entry,
        cache_root=tmp_path / ".real-e2e-cache",
        launcher_home=tmp_path / ".aionis-live-home",
    )
    ...
    assert result.details["execution_target_kind"] == "sprint"
    assert result.details["execution_summary"]
    assert result.details["changed_target_hints"]
    ...
    assert result.details["app_generate_duration_seconds"] >= 0
    assert result.details["total_duration_seconds"] >= result.details["app_generate_duration_seconds"]
