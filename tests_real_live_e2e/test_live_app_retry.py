...
from aionis_workbench.e2e.real_e2e.scenario_runner import (
    ...
    run_live_app_retry_scenario,
)


def test_live_app_retry_real_scenario(tmp_path) -> None:
    ...
    result = run_live_app_retry_scenario(
        repo_entry,
        cache_root=tmp_path / ".real-e2e-cache",
        launcher_home=tmp_path / ".aionis-live-home",
    )
    ...
    assert result.details["revision_id"] == "sprint-1-revision-1"
    assert result.details["revision_summary"]
    assert result.details["must_fix"]
    assert result.details["must_keep"]
    ...
    assert result.details["total_duration_seconds"] >= result.details["app_retry_duration_seconds"]
