...
from aionis_workbench.e2e.real_e2e.scenario_runner import (
    ...
    run_live_ab_test_second_cycle_compare_scenario,
)


def test_live_ab_test_second_cycle_report_real_scenario(tmp_path) -> None:
    ...
    result = run_live_ab_test_second_cycle_compare_scenario(
        repo_entry,
        cache_root=tmp_path / ".real-e2e-cache",
        launcher_home=tmp_path / ".aionis-live-home",
    )
    ...
    assert result.details["aionis_arm_scenario_id"] == "live-app-second-replan-generate-qa-advance"
    assert result.details["benchmark_scenario_id"] == "structured-feature-completion"
    ...
    assert result.details["aionis"]["replan_depth"] == 0
